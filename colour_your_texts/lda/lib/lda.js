...
// var stemmed = requirejs(["stem-porter"], function(util) {});
//
...
var process = function(sentences, numberOfTopics, numberOfTermsPerTopic, languages, alphaValue, betaValue, randomSeed) {
    ...
    if (sentences && sentences.length > 0) {
      ...
      languages.forEach(function(value) {
          ...
          stopwords = stopwords.concat(stopwordsLang);          
      });
      ...
      for(var i=0;i<sentences.length;i++) {
          ...
          // var doclen = documents[i].length
          // for (var j=0; j<doclen; j++){
          //   for(var k=j+1; k<doclen; k++){
          //     var ngram = vocab[documents[i][j]] + "-" + vocab[documents[i][k]]
          //     if (f[ngram]) { 
          //         f[ngram]=f[ngram]+1;
          //     } 
          //     else if(ngram) { 
          ...
          //         vocabOrig[ngram] = w;
          //     };
          //     documents[i].push(vocab.indexOf(ngram))
          //   }
          // }
      }
      ...
      var alpha = alphaValue || 0.01;  // per-document distributions over topics
      var beta = betaValue || .001;  // per-topic distributions over words
      ...
      for (var k = 0; k < phi.length; k++) {
          ...
          for (var t = 0; t < topTerms; t++) {
              var topicTerm=things[t].split("_")[2];//+"_"+things[t].split("_")[3];
              ...
              // var term = {};
              row[topicTerm] = parseFloat(things[t].split("_")[0]);
              // row.term = topicTerm;
              // row.probability = parseFloat(things[t].split("_")[0]);
              // row.push(term);
          }
...
function toFeatureVector(a){
  var v = new Array(vocab.length).fill(0);;
  for(var e of a){
    i = e['term'].split("_")[1]
    v[i] = e['probability']
  }
